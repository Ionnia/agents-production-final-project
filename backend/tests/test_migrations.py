...
def test_revision_0003_database_adds_map_point_details(tmp_path):
    database = tmp_path / "revision-0003.db"
    with sqlite3.connect(database) as connection:
        connection.execute("CREATE TABLE plan_map_points (id VARCHAR PRIMARY KEY)")
        connection.execute(
            "CREATE TABLE alembic_version (version_num VARCHAR(32) NOT NULL PRIMARY KEY)"
        )
        connection.execute("INSERT INTO alembic_version(version_num) VALUES ('20260614_0003')")
    run_alembic(database, "upgrade", "head")
    with sqlite3.connect(database) as connection:
        columns = {
            row[1] for row in connection.execute("PRAGMA table_info(plan_map_points)").fetchall()
